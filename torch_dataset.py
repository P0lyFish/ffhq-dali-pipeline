...
transform = A.Compose([
    ...
    A.OneOf([
    #     A.Downscale(scale_min=0.4, scale_max=0.7),
...
class ImageFolderDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        Author: datnvt
        """
        image, transformed_image = self._load_raw_image(self._raw_idx[idx])
        assert isinstance(image, np.ndarray)
        assert list(image.shape) == self.image_shape
        assert image.dtype == np.uint8
        assert isinstance(transformed_image, np.ndarray)
        assert list(transformed_image.shape) == self.image_shape
        assert transformed_image.dtype == np.uint8
        if self._xflip[idx]:
            assert image.ndim == 3 # CHW
            image = image[:, :, ::-1]
            assert transformed_image.ndim == 3 # CHW
            transformed_image = transformed_image[:, :, ::-1]
        return image.copy(), transformed_image.copy(), self.get_label(idx)
